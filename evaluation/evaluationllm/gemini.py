#!/usr/bin/env python3
"""
Gemini Case Law Retrieval and Evaluation Script
Uses Gemini API to find relevant case law for legal scenarios and evaluates accuracy.
"""
...
def create_case_law_prompt(scenario: str, question: str, law_type: str) -> str:
    """
    Create a focused prompt for Gemini to find relevant case law.
    """
    prompt = f"""
...
def query_gemini_for_cases(scenario: str, question: str, law_type: str) -> Dict[str, Any]:
    """
    Query Gemini API for relevant case law.
    """
    try:
...
def evaluate_single_query(query_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Query Gemini
    gemini_response = query_gemini_for_cases(scenario, question, law_type)
    predicted_cases = gemini_response.get("relevant_cases", [])
    ...
    print(f"Gemini found {len(predicted_cases)} cases ({len(valid_cases)} valid, {len(invalid_cases)} invalid)")
...
def main():
    ...
    print("Gemini Case Law Evaluation System")
    print("=" * 50)
    ...
    output_file = "evaluation/evaluationllm/gemini-1.5-flash.json"
...
if __name__ == "__main__":
    ...
    gemini_model = genai.GenerativeModel("gemini-2.0-flahs")
    main()
